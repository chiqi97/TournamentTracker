...
namespace TrackerLibrary
{
    ...
    {
        // 1 - randomizeTeams Order list randomly 
        // 2- FindNumberofRound and NumberoOfByesCheck if it is big enough - if not, add in byes
        // 2x2x2x2x2...- 2,
        // 3- CreateFirstRoundCreate our first round of matchups
        // 4- CreateOtherRounds  Creata every round after that - 8 matchups- 4 matchups- 2 matchupps- 1 matchups

        public static void CreateRounds(TournamentModel model)
        {
            List<TeamModel> randomizeTeams = RandomizeTeamOrder(model.EnteredTeams);
            int rounds = FindNumberofRound(randomizeTeams.Count);
                ...
                foreach (MatchupEntryModel entry in matchup.Entries)
                {
                    foreach (PersonModel person in entry.TeamCompeting.TeamMembers)
                    {
                        AlertPersonToNewRound(person, 
                            ...
                            matchup.Entries.Where(x=> x.TeamCompeting != entry.TeamCompeting).FirstOrDefault());
        ...
        private static void AlertPersonToNewRound(PersonModel person, 
                                                        string teamName,
        ...
        private static void MarkWinnerInMatchup(List<MatchupModel> models)
        {
            ...
            // foreach (MatchupModel m in models)
            // {
            //     if (m.Entries[0].Score != null)
            //     {
            //         teamOneScore = m.Entries[0].Score;
            //     }
        ...
        /// <summary>
        /// 1- Create randomize List<TeamModel>
        /// </summary>
